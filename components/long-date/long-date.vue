	...
	<view>
		<view class="long-data-picker" :style="'display:' + [isShow ? 'block' : 'none']">
			<picker-view :indicator-style="itemHeight" :value="dateValues" @change="bindDateChange">
				...
				<picker-view-column>
					<view class="long-datetime-item" v-for="(item,index) in dateObj.dates" :key="index">
						<picker-view-column>
							<view class="long-datetime-item" v-for="(v,k) in nearDay" :key="k">
								<text v-if="index==k">{{v}}</text>
							</view>
						</picker-view-column>
						<picker-view-column>
