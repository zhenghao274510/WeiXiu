<template>
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" style="style_Container" @click.stop>
		<!-- 输入框，仅在可输入模式下使用 -->
		<input  
			v-if="showInput"  class="input"  placeholder-style="color: #A4A4A5; font-size:11px"
			type="text" v-model="selectText"  
		...
		<!-- 列表框左上角的空心小三角 -->
			...
			>
				<div 
					class="item" @click.stop="onClickItem(index, item.name)"
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 输入框的值变化时
			onInput(event){
				const inputVal = event.detail.name;
