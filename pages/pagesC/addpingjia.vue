<template>
  <div class="add_ping">
      ...
      <li v-for="(item,index) in dataList" :key="index">
        ...
        <div class="ev_int">
          ...
          <div class="add_con">
            <div
              class="add_img"
              v-for="(itemChild,ind) in imgs[index]"
              :key="ind"
              @clickp="delimg(ind)"
            >
              <img :src="imgurl+itemChild" alt />
            </div>
            <div class="add_img" v-if="imgs[index].length!=3" @click="up(index)">
    ...
    <div class="public" :class="{'end':length==1,'more':length>1}">
      <span type="button"  class="btn" @click.prevent="getmsg">提交</span>
    </div>
  ...
  methods: {
    getmsg() {
      ...
      this.http(parmas).then(res => {
        ...
        setTimeout(() => {
          this.$router.back(-1);
          // this.$store.commit('ChangeOrdertabar',4);
        }, 1000);
      });
    },
    up(ind) {
